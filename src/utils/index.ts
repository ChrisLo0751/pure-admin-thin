...
export function formatLocalized(
  value: number | Date,
  type: "number" | "date" | "currency" = "number",
  locales = "en-US",
  options?: Intl.NumberFormatOptions | Intl.DateTimeFormatOptions
): string {
  if (type === "number") {
    return value.toLocaleString(locales, options as Intl.NumberFormatOptions);
  } else if (type === "date") {
    return value.toLocaleString(locales, options as Intl.DateTimeFormatOptions);
  } else if (type === "currency") {
    return (value as number).toLocaleString(locales, {
      style: "currency",
      ...options
    });
  } else {
